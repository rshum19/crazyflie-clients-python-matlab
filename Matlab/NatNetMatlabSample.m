                ...
                for idx = 1 : 1000   
                   ...
                   java.lang.Thread.sleep(5);  
                    ...
                    if(frameTime ~= lastFrameTime)
                        ...
                        storeData{nDifferentFrames} = mocap_data;
                        
                        if(nDifferentFrames > 4)
                            
                            % Calculate velocity
                            % Move this to the top later
                            N=2,M=5,m0=3,Dt=0.01;
                            [A,B,C,D]=num_der(N,M,m0,Dt) ;
                            C = C(2,:) ; D = D(2,:) ; % Estimate only velocity
                            % save the A,B,C,D.
                            
                            pos = [storeData{1}.pos,storeData{2}.pos,storeData{3}.pos,storeData{4}.pos];
                            x = pos(1,:)';
                            u = storeData{1}.pos(1);
                            y = C*x + D*u ;
                            x_new = A*x + D*u;
