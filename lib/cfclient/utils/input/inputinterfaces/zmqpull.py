...
class _PullReader(Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            self.cmdmsg = self._receiver.recv()
            self.roll, self.pitch, self.yaw, self.thrust = self.cmdmsg.split()
            self.cmd["ctrl"]["roll"] = float(self.roll)
            self.cmd["ctrl"]["pitch"] = float(self.pitch)
            self.cmd["ctrl"]["yaw"] = float(self.yaw)
            self.cmd["ctrl"]["thrust"] = float(self.thrust)
            self._cb(self.cmd)
